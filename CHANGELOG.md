...
- CI(`ci.yml`)に`permissions: contents: read`を明示。以前Release用に変更していた
  「リポジトリ全体のデフォルト権限=write」の影響で、フォークPRのCI実行にも書き込み権限の
  トークンが渡っていた問題を修正し、リポジトリのデフォルトも`read`に復元
- CLI履歴表示の軽微な未エスケープ箇所を多層防御として修正

### 修正
- CLIタブの実行履歴が、選択中のプラグインに関係なくアプリ全体の履歴を表示していた設計の歪みを修正。
  `cliRunner`の各実行記録に`pluginId`を付与し、プラグインごとに絞り込んで表示するように変更
