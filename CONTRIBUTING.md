...
本体はGPL-3.0ですが、GPLv3第7条の追加的許可条項（[LICENSE](../LICENSE)参照）により、
プラグイン（`plugin.json`のインターフェースのみを通じて本体と連携するもの）は
**本体のGPLに縛られず、作者が選ぶ任意のライセンス**で提供できます。
本リポジトリに同梱される公式プラグインはデフォルトで[MIT](../plugins/LICENSE)としていますが、
提案時に別のライセンスを希望する場合はIssueにその旨を記載してください。
