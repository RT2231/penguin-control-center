...
プラグインは `plugins/<プラグインid>/` 配下に以下を用意するだけで追加できます。
...
```bash
git checkout -b plugin/openssh
mkdir -p plugins/openssh
# plugin.json, docs.md を作成...
git add -A && git commit -m "plugin: OpenSSHプラグインを追加"
git push -u origin plugin/openssh
gh pr create --title "plugin: OpenSSHプラグインを追加" --body "対応ソフトウェア: OpenSSH\n動作確認環境: Ubuntu 24.04" --base main
```

マージ後、次のリリースでPlugin一覧に反映されます（現時点ではPlugin Storeは未実装のため、リポジトリ内`plugins/`への直接追加のみ対応しています）。
