...
- **更新チェック・まとめて更新**: ホーム画面で導入済みプラグインに更新があると「⬆ 更新あり」バッジが表示され、
  「まとめて更新」ボタンで一括更新できます。
- **最近のアクティビティ**: ホーム画面下部に、全プラグイン横断での直近の実行履歴を一覧表示します
  （CLIタブは選択中のプラグインの履歴のみに絞り込まれているため、全体を見渡したいときに使えます）。
  クリックするとそのプラグインのCLIタブに直接移動します。
- **パラメータ入力付きアクション**: 一部のアクション（例: Certbotの証明書発行）はGUIから
  ドメイン名等の値を入力して実行できます。入力値は正規表現でパターン検証され、実行前に
  実際に流れるコマンドがプレビュー表示されます。入力値は`execFile`の配列引数として
  「1つの引数」のまま渡されるため、シェル文字列への結合は一切発生せず、これまでと同じ
  安全性を維持しています。パスワード等の機密情報は今のところ非対応です。
